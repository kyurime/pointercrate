...
impl RankedPlayer {
    ...
    pub async fn max_index(connection: &mut PgConnection) -> Result<i64> {
        Ok(sqlx::query!("SELECT COALESCE(MAX(index), 0) AS max_index FROM players_with_score")
...
impl Player {
    ...
    pub async fn extremal_player_ids(connection: &mut PgConnection) -> Result<(i32, i32)> {
        let row = sqlx::query!("SELECT COALESCE(MAX(id), 0) AS max_id, COALESCE(MIN(id), 0) AS min_id FROM players")
            .fetch_one(connection)
            .await?;
