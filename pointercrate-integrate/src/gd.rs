...
pub type IntegrationLevel = Level<'static, CachedLevelData, Option<NewgroundsSong<'static>>>;
...
impl GeometryDashConnector {
    ...
    pub async fn lookup_level_data<'a>(&self, level_id: u64) -> Option<CachedLevelData> {
        let mut connection = self.pool.acquire().await.ok()?;
